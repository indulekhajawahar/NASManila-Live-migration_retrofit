...
//        textViewCCAaItem.setText(Html.fromHtml(PreferenceManager.getCCATitle(mContext) + "<br/>" + PreferenceManager.getStudNameForCCA(mContext)));
    ...
    private void ccaReviewListAPI() {
        ...
        paramObject.addProperty("student_id", PreferenceManager.getStudIdForCCA(mContext));
        paramObject.addProperty("cca_days_id", PreferenceManager.getCCAItemId(mContext));
        Call<CcaReviewResponseModel> call = service.cca_reviews("Bearer "+PreferenceManager.getAccessToken(mContext),paramObject);
        call.enqueue(new Callback<CcaReviewResponseModel>()
        {
            @Override
            public void onResponse(Call<CcaReviewResponseModel> call, Response<CcaReviewResponseModel> response) {
                if (response.isSuccessful()) {
                    CcaReviewResponseModel apiResponse = response.body();
                            ...
                            if (apiResponse.getResponse().getData().size() > 0) {
                                for (int j = 0; j < weekList.size(); j++) {
                                    for (int i = 0; i < apiResponse.getResponse().getData().size(); i++) {
                                        CCAReviewAfterSubmissionModel item = apiResponse.getResponse().getData().get(i);
                                        Gson gson = new Gson();
                                        String eventJson = gson.toJson(item);
                                        try {
                                            JSONObject jsonObject = new JSONObject(eventJson);
                                             Log.e("Json", String.valueOf(jsonObject));
                                            if (jsonObject.optString("day").equalsIgnoreCase(weekList.get(j).toString())) {
                                                mCCADetailModelArrayList.add(addCCAReviewlist(jsonObject));
                                            }
            ...
            @Override
            public void onFailure(Call<CcaReviewResponseModel> call, Throwable t) {
