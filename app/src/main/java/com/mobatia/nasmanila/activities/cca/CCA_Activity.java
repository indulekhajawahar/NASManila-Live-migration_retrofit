        ...
        recycler_review.addOnItemTouchListener(new RecyclerItemListener(getApplicationContext(), recycler_review,
                new RecyclerItemListener.RecyclerTouchListener() {
                    public void onClickItem(View v, int position) {
                        Log.e("submissiondateover",mCCAmodelArrayList.get(position).getIsSubmissionDateOver());
...
//                                    studentList.add(studentsModelArrayList.get(i).getmName());
    ...
    private void getCCAListAPI(final String studentId) {
        ...
        Call<CcaDetailsResponseModel> call = service.cca_details("Bearer "+PreferenceManager.getAccessToken(mContext),paramObject);
        call.enqueue(new Callback<CcaDetailsResponseModel>()
        {
            @Override
            public void onResponse(Call<CcaDetailsResponseModel> call, Response<CcaDetailsResponseModel> response) {
                if (response.isSuccessful()) {
                    CcaDetailsResponseModel apiResponse = response.body();
                    String responsCode = apiResponse.getResponsecode();
                    if (responsCode.equals("200")) {
                        ...
                        if (statusCode.equals("303")) {
                            if (apiResponse.getResponse().getData().size() > 0) {
                                for (int i = 0; i < apiResponse.getResponse().getData().size(); i++) {
                                    CCAModel item = apiResponse.getResponse().getData().get(i);
                                    Gson gson = new Gson();
                                    String eventJson = gson.toJson(item);
                                    try {
                                        JSONObject jsonObject = new JSONObject(eventJson);
            ...
            @Override
            public void onFailure(Call<CcaDetailsResponseModel> call, Throwable t) {
                //progressDialogP.hide();
                Log.e("toadst", String.valueOf(t));
                //Toast.makeText(mContext, "Successfully sent email to staff"+t, Toast.LENGTH_SHORT);
