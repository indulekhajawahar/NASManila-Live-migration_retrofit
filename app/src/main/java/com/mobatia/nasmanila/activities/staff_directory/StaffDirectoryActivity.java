...
//	ViewPager bannerImagePager;
	...
	private void callStaffDirectoryListAPI(String URL) {
		...
		Call<StaffCategoriResponseModel> call = service.getstaffcategorylist("Bearer "+PreferenceManager.getAccessToken(mContext));
		call.enqueue(new Callback<StaffCategoriResponseModel>()
		{
			@Override
			public void onResponse(Call<StaffCategoriResponseModel> call, Response<StaffCategoriResponseModel> response) {
				if (response.isSuccessful()) {
					StaffCategoriResponseModel apiResponse = response.body();
...
//								bannerImagePager.setAdapter(new ImagePagerDrawableAdapter(bannerUrlImageArray, mContext));
			...
			@Override
			public void onFailure(Call<StaffCategoriResponseModel> call, Throwable t) {
