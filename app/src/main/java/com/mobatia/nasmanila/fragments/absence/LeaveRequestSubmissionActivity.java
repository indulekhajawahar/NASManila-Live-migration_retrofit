...
//                    pickerIOS.setVisibility(View.GONE);
    ...
    private void submitLeave(String URL_API) {
        ...
        paramObject.addProperty("from_date", fromDate);
        paramObject.addProperty("to_date", toDate);
        paramObject.addProperty("reason", enterMessage.getText().toString().trim());
        Call<CcaSubmitResponseModel> call = service.requestLeave("Bearer "+PreferenceManager.getAccessToken(mContext),paramObject);
