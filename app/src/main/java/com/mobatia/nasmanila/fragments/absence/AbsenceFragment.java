...
boolean firstVisit;
    ...
    public void getList(final String URL, final String student_id) {
        ...
        paramObject.addProperty("student_id", student_id);
        paramObject.addProperty("users_id", PreferenceManager.getUserId(mContext));

        Call<LeaveRequestResponseModel> call = service.leaveRequests("Bearer "+PreferenceManager.getAccessToken(mContext),paramObject);
        call.enqueue(new Callback<LeaveRequestResponseModel>()
        {
            @Override
            public void onResponse(Call<LeaveRequestResponseModel> call, Response<LeaveRequestResponseModel> response) {
                if (response.isSuccessful()) {
                    Log.e("Successt","Success");

                    LeaveRequestResponseModel apiResponse = response.body();
                    ...
                    if (responsCode.equals("200")) {
                            ...
                            if (apiResponse.getResponse().getData().size() > 0) {
                                for (int i = 0; i < apiResponse.getResponse().getData().size(); i++) {
                                   ...
                                   // JSONObject dataObject = dataArray.optJSONObject(i);
                                    LeavesModel dataObject=apiResponse.getResponse().getData().get(i);
                                    LeavesModel mLeavesModel = new LeavesModel();
                                    mLeavesModel.setTo_date(dataObject.getTo_date());
                                    mLeavesModel.setFrom_date(dataObject.getFrom_date());
                                    mLeavesModel.setReason(dataObject.getReason());
                                    mLeavesModel.setStatus(dataObject.getStatus());
                        ...
                        }else if(statusCode.equals("313")){
                            AppUtils.showDialogAlertDismiss((Activity) mContext, "Alert", "Already Request", R.drawable.exclamationicon, R.drawable.round);
            ...
            @Override
            public void onFailure(Call<LeaveRequestResponseModel> call, Throwable t) {
...
//								CustomStatusDialog(RESPONSE_FAILURE);
    ...
    private void getStudentsListFirstAPI(final String URLHEAD) {
        APIInterface service = APIClient.getRetrofitInstance().create(APIInterface.class);
        JsonObject paramObject = new JsonObject();
        paramObject.addProperty(JTAG_USERS_ID, PreferenceManager.getUserId(mContext));
        Call<StudentResponseModel> call = service.studentlist("Bearer "+PreferenceManager.getAccessToken(mContext),paramObject);
        call.enqueue(new Callback<StudentResponseModel>()
        {
            @Override
            public void onResponse(Call<StudentResponseModel> call, Response<StudentResponseModel> response) {
                if (response.isSuccessful()) {
                    StudentResponseModel apiResponse = response.body();
                    String responsCode = apiResponse.getResponsecode();
                    if (responsCode.equals(RESPONSE_SUCCESS)) {
                        String statusCode = apiResponse.getResponse().getStatuscode();
                        ...
                        if (statusCode.equals(STATUS_SUCCESS)) {
                            ...
                            if (apiResponse.getResponse().getData().size() > 0) {
                                for (int i = 0; i < apiResponse.getResponse().getData().size(); i++) {
                                    StudentModel item = apiResponse.getResponse().getData().get(i);
                                    Gson gson = new Gson();
