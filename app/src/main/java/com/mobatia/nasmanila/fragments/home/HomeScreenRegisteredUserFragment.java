...
//		}
	...
	public void getBanner() {
		...
		paramObject.addProperty("app_version", "2.3");
		paramObject.addProperty("users_id", "21");
		paramObject.addProperty("devicetype", "2");
		Call<BannerResponseModel> call = service.bannerresponse("Bearer "+PreferenceManager.getAccessToken(mContext),paramObject);
		call.enqueue(new Callback<BannerResponseModel>()
		{
			...
			public void onResponse(Call<BannerResponseModel> call, Response<BannerResponseModel> response) {
				if (response.isSuccessful()) {
					BannerResponseModel apiResponse = response.body();
...
//												JSONObject dataObject = dataArray.getJSONObject(i);
										Log.e("data3", String.valueOf(apiResponse.getResponse().getData().get(i)));

										homeBannerUrlImageArray.add(apiResponse.getResponse().getData().get(i));
									}
									bannerImagePager.setAdapter(new ImagePagerDrawableAdapter(homeBannerUrlImageArray, getActivity()));
...
//											Toast.makeText(mContext, "Failure", Toast.LENGTH_SHORT).show();
								...
								android_app_version = apiResponse.getResponse().getAndroid_app_version();
