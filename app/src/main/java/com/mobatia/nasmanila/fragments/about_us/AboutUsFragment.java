...
//		}else
	...
	public void getList() {
		...
		Call<AboutUsResponseModel> call = service.about_us("Bearer "+PreferenceManager.getAccessToken(mContext));
		call.enqueue(new Callback<AboutUsResponseModel>()
		{
			...
			public void onResponse(Call<AboutUsResponseModel> call, Response<AboutUsResponseModel> response) {
				if (response.isSuccessful()) {
					AboutUsResponseModel apiResponse = response.body();
					...
					if (responsCode.equals(RESPONSE_SUCCESS)) {
						...
						if (statusCode.equals(STATUS_SUCCESS)) {
							if (statusCode.equals(STATUS_SUCCESS)) {
								...
								/*AboutusModel respObject = apiResponse.getResponse().getData();
								...
								email_nas=apiResponse.getResponse().getContact_email();
								description.setText(apiResponse.getResponse().getDescription());
								weburl.setText(apiResponse.getResponse().getWebsite_link());
								weburlString=apiResponse.getResponse().getWebsite_link();
								descriptionString=apiResponse.getResponse().getDescription();
								...
								Log.e("aboutusarray", String.valueOf(apiResponse.getResponse().getData().size()));
								System.out.println("Array"+apiResponse.getResponse().getData().get(0).getId());
								if (apiResponse.getResponse().getData().size() > 0) {
									for (int i = 0; i < apiResponse.getResponse().getData().size(); i++) {
										if (i>=0) {
											AboutusModel item = apiResponse.getResponse().getData().get(i);
											Gson gson = new Gson();
...
//												}
	...
	private AboutusModel getSearchValues(JSONObject Object)
		...
		AboutusModel mAboutUsModel = new AboutusModel();
		mAboutUsModel.setId(Object.getInt(JTAG_ID));
...
//            listItem.measure(0, 0);
	...
	private void sendEmailToStaff(String URL, Dialog dialog) {
		...
		paramObject.addProperty("email", email_nas);
