...
//		bannerImagePager= (ViewPager) mRootView.findViewById(R.id.bannerImageViewPager);
	...
	public void getList() {
		...
		Call<NAsTodayResponseModel> call = service.nastoday("Bearer "+PreferenceManager.getAccessToken(mContext));
		call.enqueue(new Callback<NAsTodayResponseModel>()
		{
			@Override
			public void onResponse(Call<NAsTodayResponseModel> call, Response<NAsTodayResponseModel> response) {
				if (response.isSuccessful()) {
					NAsTodayResponseModel apiResponse = response.body();
...
//											bannerImagePager.setAdapter(new ImagePagerDrawableAdapter(bannerUrlImageArray, getActivity()));
							...
							if (apiResponse.getResponse().getData().size()> 0) {
								for (int i = 0; i <apiResponse.getResponse().getData().size(); i++) {
									NasTodayModel item = apiResponse.getResponse().getData().get(i);
									Gson gson = new Gson();
			...
			@Override
			public void onFailure(Call<NAsTodayResponseModel> call, Throwable t) {
