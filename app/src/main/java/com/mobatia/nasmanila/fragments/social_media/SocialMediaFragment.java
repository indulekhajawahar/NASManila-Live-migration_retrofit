...
//						startActivity(i);
    ...
    private void callSocialMediaListAPI(String URL) {
        ...
        Call<SocialMediaResponseModel> call = service.social_media("Bearer "+PreferenceManager.getAccessToken(mContext));
        call.enqueue(new Callback<SocialMediaResponseModel>()
        {
            @Override
            public void onResponse(Call<SocialMediaResponseModel> call, Response<SocialMediaResponseModel> response) {
                if (response.isSuccessful()) {
                    SocialMediaResponseModel apiResponse = response.body();
                            ...
                            if (apiResponse.getResponse().getData().size() > 0) {
                                for (int i = 0; i < apiResponse.getResponse().getData().size(); i++) {
                                   // JSONObject dataObject = data.getJSONObject(i);
                                    SocialMediaModel dataObject=apiResponse.getResponse().getData().get(i);
                                    SocialMediaModel socialMediaModel = new SocialMediaModel();
                                    socialMediaModel.setId(dataObject.getId());
                                    socialMediaModel.setUrl(dataObject.getUrl());
                                    socialMediaModel.setTab_type(dataObject.getTab_type());
                                    socialMediaModel.setImage(dataObject.getImage());
                                    if (dataObject.getTab_type().contains("Facebook")) {
                                        mSocialMediaArraylistFacebook.add(socialMediaModel);
                                    }
                                    else if(dataObject.getTab_type().contains("Twitter")){
                                        mSocialMediaArraylistTwitter.add(socialMediaModel);
                                        //mSocialMediaArray=mSocialMediaArraylistTwitter;
                                    }else if (dataObject.getTab_type().contains("Instagram")) {
