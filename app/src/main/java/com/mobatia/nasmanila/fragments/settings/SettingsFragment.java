...
//		}
    ...
    private void callChangePasswordAPI(String URL) {
        ...
        paramObject.addProperty("current_password", text_currentpswd.getText().toString());
        paramObject.addProperty("new_password", newpassword.getText().toString());
        paramObject.addProperty("email", PreferenceManager.getUserEmail(mContext));
        paramObject.addProperty("deviceid", PreferenceManager.getFCMID(mContext));
        paramObject.addProperty("devicetype", "2");

        Call<CcaSubmitResponseModel> call = service.changepassword("Bearer "+PreferenceManager.getAccessToken(mContext),paramObject);
