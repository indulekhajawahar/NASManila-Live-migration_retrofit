...
//		setHasOptionsMenu(true);
	...
	private void callcontactUsApi(String URL) {
		...
		Call<ContactResponseModel> call = service.contact_us("Bearer "+PreferenceManager.getAccessToken(mContext));
		call.enqueue(new Callback<ContactResponseModel>()
		{
			...
			public void onResponse(Call<ContactResponseModel> call, Response<ContactResponseModel> response) {
				if (response.isSuccessful()) {
					ContactResponseModel apiResponse = response.body();
					...
					if (responsCode.equals("200")) {
						...
						if (statusCode.equals("303")) {
							...
							latitude = apiResponse.getResponse().getData().getLatitude();
							longitude = apiResponse.getResponse().getData().getLongitude();
							description = apiResponse.getResponse().getData().getDescription();
							desc.setText(description);
							if (apiResponse.getResponse().getData().getContacts().size() > 0) {
								for (int i = 0; i <=apiResponse.getResponse().getData().getContacts().size(); i++) {
								//	System.out.println("length"+contact.length());
									if (i<apiResponse.getResponse().getData().getContacts().size())
									{
										ContactUsModel questionsObject = apiResponse.getResponse().getData().getContacts().get(i);

										System.out.println("working");
										//JSONObject cObj = contact.getJSONObject(i);
										ContactUsModel contactUsModel = new ContactUsModel();
										contactUsModel.setContact_email(questionsObject.getContact_email());
										contactUsModel.setContact_phone(questionsObject.getContact_phone());
										contactUsModel.setContact_name(questionsObject.getContact_name());
										contactUsModelsArrayList.add(contactUsModel);
									}
									else if (i==apiResponse.getResponse().getData().getContacts().size())
									{
										System.out.println("working ****");
										ContactUsModel contactUsModel = new ContactUsModel();
