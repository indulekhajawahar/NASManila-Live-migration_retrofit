...
*/
    ...
    public void getStaffList(String student_id) {
        ...
        APIInterface service = APIClient.getRetrofitInstance().create(APIInterface.class);
        JsonObject paramObject = new JsonObject();
        paramObject.addProperty(JTAG_STUDENT_ID, student_id);
        Call<StaffResponseModel> call = service.stafflist("Bearer "+PreferenceManager.getAccessToken(mContext),paramObject);
        call.enqueue(new Callback<StaffResponseModel>()
        {
            @Override
            public void onResponse(Call<StaffResponseModel> call, Response<StaffResponseModel> response) {
                if (response.isSuccessful()) {
                    StaffResponseModel apiResponse = response.body();
                    String responsCode = apiResponse.getResponsecode();
                    if (responsCode.equals(RESPONSE_SUCCESS)) {
                        String statusCode = apiResponse.getResponse().getStatuscode();
                        ...
                        if (statusCode.equals(STATUS_SUCCESS)) {
                            if (apiResponse.getResponse().getData().size() > 0) {
                                for (int i = 0; i < apiResponse.getResponse().getData().size(); i++) {
                                    StaffModel item = apiResponse.getResponse().getData().get(i);
                                    Gson gson = new Gson();
                                    String eventJson = gson.toJson(item);
                                    try {
                                        JSONObject jsonObject = new JSONObject(eventJson);
...
//								CustomStatusDialog(RESPONSE_FAILURE);
    ...
    private void sendEmailToStaff(String URL) {
        ...
        APIInterface service = APIClient.getRetrofitInstance().create(APIInterface.class);
        JsonObject paramObject = new JsonObject();
        paramObject.addProperty("studentid", mStudentId);
        paramObject.addProperty("staffid", mStaffId);
        paramObject.addProperty("parentid", PreferenceManager.getUserId(mContext));
        paramObject.addProperty("title", text_dialog.getText().toString());
        paramObject.addProperty("description", text_content.getText().toString());
        Call<CcaSubmitResponseModel> call = service.pta_meeting("Bearer "+PreferenceManager.getAccessToken(mContext),paramObject);
